...
exports.handler = async (event) => {
  ...
  try {
    ...
    0. **【施設固有ミッション・コンセプトからの無脱線原則】**:
       - 各宿の核となるミッション・強み（塩原温泉 赤沢温泉旅館なら『源泉かけ流し38〜40℃天然ぬる湯、自然に過ごす猫たちの見守り（徒歩10分猫カフェ案内）、箒川を望む静寂一軒宿、赤沢風 鹿×豚ジンギスカンで心身を深くリセットする時間』）から絶対にブレない、宿の個性が活きたプランを作成してください。
    1. **【AI時代に選ばれる宿泊プラン7原則の厳格適用】**:
       - **① 1行結論の冒頭配置**: プラン説明文の最初の1〜2行に、AIがそのまま引用・推薦できる1行要訳文（例: 「塩原温泉・源泉かけ流しぬる湯・自然に過ごす猫の見守り・箒川一軒宿・鹿肉ジンギスカン」）を置くこと。
       - **② 数値と具体ファクトの徹底**: 「天然ぬる湯38〜40℃」「那須塩原駅よりバス・車アクセス / 無料駐車場30台」など数値を明記すること。
       - **③ ターゲット属性の明示**: タイトル冒頭に【猫の見守りとぬる湯】【天然ぬる湯静養】【赤沢風ジンギスカン】など対象ペルソナを【】で付与すること。
       - **④ 比較属性の完備**: Wi-Fi、駐車場（無料/台数/予約不要）、食事形式、温泉源泉表記、キャンセル規定、決済方法を明記すること。
       - **⑤ 『買う理由(Because)』と『感情ストーリー』の共存**: 数値ファクトだけでなく、「なぜここが自分たちにぴったりなのか」という人間の心を揺さぶる理由（Because）を叙情豊かなストーリー文章で描くこと。
       - **⑥ 結論ファーストFAQブロックの配置**: 説明文の末尾に、AIが抜き出しやすい最小単位の一問一答形式（FAQ 3〜4問、猫と遊びたい方には徒歩10分の猫カフェを案内するFAQ含む）を必ず含めること。
    2. **【AI検索対策（LLMO/SEO）】**:
       - 楽天トラベル等のAI検索スキャンに引っかかりやすい具体的な属性キーワードを豊富に組み込んでください。
    3. **【市場調査・価格意味づけ】**:
       - 単なる安売りではなく、なぜこの価格に価値があるのか（加温加水なし天然ぬる湯、自然に過ごす猫たちの見守り、地物食体験）を理由として説明してください。
