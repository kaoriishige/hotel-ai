...
function renderChannelSettings(row) {
  ...
  return channels.map(channel => {
    ...
    if (channel === 'instagram_reel' && narrationText) {
...
modalApproveBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    const response = await fetch(`${apiBase}/approve-post`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: currentPreviewId, action: 'approve', channel: 'instagram_reel' })
    });
    if (!response.ok) {
      const data = await response.json();
    ...
    alert('動画(Instagram リール)を承認しました。');
    closePreviewModal();
    await loadQueue();
  } catch (error) {
    alert(error.message);
...
modalPublishNowBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    const response = await fetch(`${apiBase}/approve-post`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: currentPreviewId, action: 'publish_now', channel: 'instagram_reel' })
    });
    if (!response.ok) {
      const data = await response.json();
    ...
    alert('動画(Instagram リール)を即時投稿しました！');
    closePreviewModal();
    await loadQueue();
  } catch (error) {
    alert(error.message);
