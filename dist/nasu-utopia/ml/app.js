...
function buildMessage(customer, channelOverride) {
  ...
  const customerWithFullName = {
    ...customer,
    name,
    greeting: (name === '那須ユートピア美野沢ご利用者様' || name === '未登録 様') ? '那須ユートピア美野沢ご利用者様' : `${name} 様`
  };
  ...
  const subject = (el.customSubject && el.customSubject.value.trim()) || tpl.emailSubject || '【那須ユートピア美野沢】ご案内';
  return { subject, body };
}
