...
if (generateAvatarVideoBtn) {
  generateAvatarVideoBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      // AIへの背景・演出指示テキストを取得
      const motionPrompt = document.getElementById('avatarMotionPrompt')?.value?.trim() || '';
...
async function loadTrendSuggestions() {
  ...
  try {
    ...
    listEl.innerHTML = suggestions.map(s => {
      const cleanTitle = escapeHtml(s.title).replace(/本格フィンランドサウナ（CUBERU / Rekka）と那須連山の水風呂/g, '静けさ').replace(/サウナ・水風呂/g, '自分時間').replace(/那須ユートピアの?/g, '');
      const cleanTheme = escapeHtml(s.theme).replace(/本格フィンランドサウナ（CUBERU / Rekka）と那須連山の水風呂/g, '静けさ').replace(/サウナ・水風呂/g, '自分時間').replace(/那須ユートピアの?/g, '');
      const cleanReason = escapeHtml(s.reason).replace(/本格フィンランドサウナ（CUBERU / Rekka）と那須連山の水風呂/g, '静けさ').replace(/サウナ・水風呂/g, '自分時間').replace(/那須ユートピアの?/g, '');
...
// 💬 AIへの背景・演出指示クイック入力ボタン
document.querySelectorAll('.prompt-quick-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    const promptInput = document.getElementById('avatarMotionPrompt');
